...
async function getDownloadURL ({ channel_id, subscriber_id }, release) {
  ...
  const params = {
    Key: `channels/${_channel_id}/releases/${time_created}/payload/${payload}`,
    Bucket: process.env.S3_BUCKET_OUT,
    Expires: 3600
  };

  if (channel_id && channel_id === release.channel_id) {

    // Allow channel to request own download link.

    return promisify(callback => S3.getSignedUrl("getObject", params, callback));
  }

  // Check subscriber permissions.

  const subscription = await getSubscriptionByChannelID(subscriber_id, _channel_id);

  if (!subscription) {
    console.log("no subscription")
    throw new Error("![403] Unauthorized.");

  } else if (tier > subscription.tier) {
    console.log(tier)
...
function getTransfersByChannelID (channel_id) {
...
async function getTransfersBySyndicateID (syndicate_id, channel_id) {

  await syndicateHasChannel(syndicate_id, channel_id);

  return queryAll({
    TableName: process.env.DYNAMODB_TABLE_TRANSFERS,
    IndexName: `${process.env.DYNAMODB_TABLE_TRANSFERS}-GSI`,
    KeyConditionExpression: "syndicate_id = :syndicate_id",
    ExpressionAttributeValues: { ":syndicate_id": syndicate_id }
  });
}
