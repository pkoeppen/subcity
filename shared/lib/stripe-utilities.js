...
async function handleSubscriptionRateChange({ channel_id, syndicate_id }, rate) {

  // Create a new Stripe plan associated with the (channel|syndicate)'s Stripe product,
  // delete the old plan, and move all subscribers to the new plan if so specified.

  var type, product, channel, syndicate;

  if (channel_id) {
    channel = await getChannelById(channel_id);
    product = `prod_channel_${channel_id}`;
  } else if (syndicate_id) {
    syndicate = await getSyndicateById(syndicate_id);
  ...
  console.log(`[Stripe] Creating new plan for ${product}`);
  ...
  if (true) {
    ...
    await handleSubscriptionTransfer(product, new_plan_id);
  }
