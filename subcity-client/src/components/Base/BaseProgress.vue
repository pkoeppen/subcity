<template>
  <div class="progress-wrapper">
    <div :class="`progress-${type}`">
      ...
      <div v-if="percentage" class="progress-percentage">
        <slot>
          <span>{{value}}%</span>
        </slot>
      </div>
    ...
    <div class="progress-label text-center text-sm-right" v-if="label">
      <slot name="label">
        <span>{{label}}</span>
      </slot>
    </div>
...
export default {
  ...
  props: {
    percentage: {
      type: Boolean,
      description: "Whether to show the progress % above the bar",
      default: true
    },
    striped: {
      type: Boolean,
